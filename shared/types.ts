...
export interface DockerInspect {
  ...
  command: string
  /**
   * Pre-formatted for display ("" = no usable timestamp) - a block's
   * `format` has no date/time kind, and a `keyValue` row cannot run
   * `Date(ms).toLocaleString()` itself. Rendered with the server's own
   * locale/timezone rather than the viewer's, since this is computed once
...
/** Written by the update script and shown once on the next start. */
...
export interface ServiceEntry {
   ...
   */
  estCostPct?: number
  /** % - present on `self`, and on any stream/shell entry with a pid */
  cpu?: number
  memBytes?: number
}
